...
cxxopts::Options get_options(int nargs, char* argv[]) {
    ...
    opt.add_options()
        ...
        ("P,pypy", "use pypy to run python", cxxopts::value<bool>()->default_value("false"))
...
//         bconf.set("tests", args["i"].as<std::vector<std::string>>());
        ...
        bconf.set("use_pypy", args["P"].as<bool>());
    }
    ...
    try {
        ...
        for (const std::string& sol_name: sol_names) {
            threads.emplace_back([&] () {
                ...
                try {
                    Solution sol(sol_name, bconf.get<double>("time_limit"), bconf.get<bool>("use_pypy"));
